...
# calculate weighted SPRT
calc_wt_sprt <- function(data) {
  data <-
  data %>%
  group_by(simulation) %>%
  mutate(wt = 0.3,
          lagval  = lag(sr)) %>%
  rowwise() %>%
  mutate(wsprt = sum( (wt * sr) + ((1 - wt) * lagval), na.rm = TRUE),
         decision_wt = case_when(wsprt <= a & wsprt >= b ~"?",
                              wsprt > a ~ "H1",
                              wsprt < b ~ "H0")) %>%
  select(-lagval)
  return(data)
...
where $Z_i$ is the number of secondary cases produced by the $i$-th infected individual which follows a negative binomial distribution with mean $R_t$ and dispersion $k$. Epidemiologically, $R_t$ is the time-specific mean number of infections caused by an infectious individual and $k$ represents variability in infectiousness among individuals [@lloyd-smith_superspreading_2005]. Under the assumption that all $Z_i$ are independent and identically distributed (iid) the total number of secondary infections is given by

\begin{align*}
X_{t+1} & \sim \Sigma^{X_t}_{i=1}Z_i \\
            & \sim NB(\Sigma^{X_t}_{i=1} R_t, \Sigma^{X_t}_{i=1} k)
\end{align*}
...
```{r branching_model, eval = FALSE, echo = TRUE}
episim <- function (t, i_0, r0, disp, cont, nsim) {
...
```{r extinction_prob, cache = TRUE, echo = TRUE} 
...
# probability generating function for NB
...
# calculate g_n(0) as n -> niteration. This is the probality of extinction when there is initially 1 infected
x <- 1
...
```{r simulate_final_size, cache = TRUE, eval = FALSE}
...
In our case, the parameter ($\theta$) of interest is $R_t$. When $S_r > \log(A)$ the decision maker concludes that it is unsafe to reopen at time $r$ and when $S_r \leq \log (B)$ the decision maker concludes it is safe to reopen at time $r$. This should have an advantage over the criteria that have been proposed on purely heuristic grounds since the decision maker is able to set choose the type I and II error rates whereas the error rates of the proposed criteria are uncontrolled and unknown. Another likely justification is that the SPRT may be the optimal test for this problem class (it is for the case where observations are iid, not sure about the markov dependence case).
...
R_t = 
    \begin{cases}
      2  & \text{when} & t = 1, \ldots, 10\\
      0.9 & \text{when} & t = 11, \ldots, 20
    \end{cases}
\end{align*}

and test the hypothesis $H_0: R_t > 1 \text{ against } H_1: R_t \leq 1$. This means that $H_0$ is true when $t = 1, \ldots, 10$ and $H_1$ is true when $t = 11, \ldots ,20$.
...
```
**Figure 3.** Panels a and b show decisions returned by RSPRT for each simulation when the initial outbreak size is 10 (a) vs. 100 (b). Panels c and d show proportion of simulations where a decision was returned by the RSPRT for initial outbreak sizes of 10 (c) or 100 (d). The vertical black dashed line at generation 11 denotes the point that $H_1$ becomes true. Results shown are for `r n` independent simulations.

The RSPRT is less able to reject the null hypothesis $(R_t > 1)$ when infected generation sizes are smaller. This may be caused by demographic stochasticity which is most influential in small populations and can create fluctuations in infected generation size (see simulations in bottom of 3a). I suspect that in smaller outbreak power is also reduced because there is less of a difference in the probability of decreased infected generation sizes under either hypothesis when the infected generation sizes are small (due to overdispersion of negative binomial).

The increase in statistical power based on infected generation size is evident when comparing figures 3c and 3d. Under more severe outbreaks (3d) RSPRT rejects the null hypothesis for nearly every time step after $H_1$ becomes true. Under the less severe outbreaks (3c) around half of the simulations rejects the null hypothesis at each time step after $H_1$ becomes true.

However, it is likely that a more reasonable assessment of the RSPRT is to calculate the proportion of simulations for which $H_0$ is appropriately rejected at least once and the distribution of lag times.

### Case 2: Linearly decreasing $R_t$

How do the sequential tests perform when $R_t$ decreases linearly $R_t = 2.1 - 0.1t$ where $t = 1, \ldots , 20$. 

We test the hypothesis that $H_0: R_t > 1-c \text{ against } H_1: R_t \leq 1-c$. That is, when is transmission low enough that repoening will not result in $R > 1$. In this case $c = 0.3$ so the threshold is $0.7$. This means that $H_1$ is true when $t = 14, \ldots , 20$.
...
**Figure 4: Performance of reset SPRT when** $\boldsymbol{R_t}$ **decreases linearly** Panels a and b show decisions returned by RSPRT for each simulation when the initial outbreak size is 10 (a) vs. 100 (b). Panels c and d show proportion of simulations where a decision was returned by the RSPRT for initial outbreak sizes of 10 (c) or 100 (d). The vertical line at generation 11 denotes when $R_t = 1$ and the vertical dashed line at generation 14 denotes the point that $H_1$ becomes true. Results shown are for `r n` independent simulations.

The effect of outbreak severity on the ability of RSPRT to reject $H_0$ in an appropriate and timely manner is similar to the case 1 for the breakpoint change in $R_t$. However, there is now a marked delay of one generation in the majority of simulations before RSPRT reject $H_0$ (figure 4 c and d). There is also a mode in proportion of simluations rejecting $H_0$ at generation 18 in both outbreak severity scenarios. The factors influencing this will be important to quantify. It would be interesting to examine how the rate and functional form of the trend in $R_t$ influences this time to detection.

# Comparing weighted SPRT (WSPRT) and reset SPRT (RSPRT)

Here, I compared the performance of WSPRT and RSPRT for low (100) and high (500) initial infected. In all figures, top panels are high initial infected and bottom panels are low initial infected.

```{r simulate_date_wsprt, cache = TRUE}
sim_t <- seq.int(1, 40, by = 1)
r_linear <- seq(2, 0.6, by = -0.1)
r_const <- rep(0.6, time = length(sim_t) - length(r_linear))

r_seq <- c(r_linear, r_const)
...
set.seed(123)
test <- episim(t = sim_t, i_0 = 5*10^2, r0 = r_seq, disp = disp, cont = cont, nsim = n)
test_low <- episim(t = sim_t, i_0 = 10^2, r0 = r_seq, disp = disp, cont = cont, nsim = n)

```


```{r calc_wsprt}
test_sprt <- calc_sr(test, accept = acceptance, reject = rejection)
...
test_sprt <-
  ...
  mutate(sr = log(lik1/lik0) ,
         ...
         decision = case_when(sprt <= a & sprt >= b ~"?",
                              sprt > a ~ "H1",
                              sprt < b ~ "H0"))
test_sprt <- calc_reset_sr(test_sprt)

test_sprt_low <- calc_sr(test_low, accept = acceptance, reject = rejection)
...
test_sprt_low <-
  ...
  mutate(sr = log(lik1/lik0) ,
         ...
         decision = case_when(sprt <= a & sprt >= b ~"?",
                              sprt > a ~ "H1",
                              sprt < b ~ "H0"))
test_sprt_low <- calc_reset_sr(test_sprt_low)
...
test_sprt <- calc_wt_sprt(test_sprt)
test_sprt_low <- calc_wt_sprt(test_sprt_low)
...
Detection delay is the number of generations between the time that $H_1$ is first rejected and the time that $H_1$ first became true. In general, RSPRT has a smaller detection delay than WSPRT. When the outbreak is large enough (top panel) both tests perform similarly. However,for smaller outbreaks (bottom panel) there are simualtions for both tests where the null hypothesis is never rejected before the end of the outbreak. For the RSPRT about 20 % of simulations fail to reject the null; for the WSPRT about 30 % of simulations fail to reject the null. These are both much larger type II error rates than specified (here, $\beta = 0.05$) using the Wald approximations for the decision thresholds A and B. In the later plots it is clear that type I error is close to 0 which is much smaller than specified ($\alpha = 0.05$). This does leave room for the possibility that narrower boundaries would be appropriate since there is room for additional type I error which will also improve the power of the tests.

```{r out.width="50%", fig.show="hold"}
is_h1 <- function(x) x == "H1"
is_thresh_R <- function(x) x == thresh_R

delay_reset <- test_sprt %>% ungroup() %>%
  select(time, R0_true, simulation, decision_reset) %>%
  group_by(simulation) %>%
  mutate(detect_time = detect_index(decision_reset, is_h1),
         detect_time = ifelse(detect_time > 0, detect_time, 10^6),
         true_time = detect_index(R0_true, is_thresh_R)) %>%
  summarize(delay = unique(detect_time - true_time)) 

delay_reset %>% pull(delay) %>% ecdf() %>% 
  plot(xlim = c(1, 10), ylim = c(0, 1), 
       main = "Distribution of detection delay for RSPRT", ylab = "P(reject null | null is false)", xlab = "detection lag")

delay_wt <- test_sprt %>% ungroup() %>%
  select(time, R0_true, simulation, decision_wt) %>%
  group_by(simulation) %>%
  mutate(detect_time = detect_index(decision_wt, is_h1),
         detect_time = ifelse(detect_time > 0, detect_time, 10^6),
         true_time = detect_index(R0_true, is_thresh_R)) %>%
  summarize(delay = unique(detect_time - true_time)) 

delay_wt %>% pull(delay) %>% ecdf() %>% 
  plot(xlim = c(1, 10), ylim = c(0, 1), 
       main = "Distribution of detection delay for WSPRT", ylab = "P(reject null | null is false)", xlab = "detection lag")
  
```


```{r out.width = "50%", fig.show = "hold"}
delay_reset_low <- test_sprt_low %>% ungroup() %>%
  select(time, R0_true, simulation, decision_reset) %>%
  group_by(simulation) %>%
  mutate(detect_time = detect_index(decision_reset, is_h1),
         detect_time = ifelse(detect_time > 0, detect_time, 10^6),
...
delay_reset_low %>% pull(delay) %>% ecdf() %>% 
  plot(xlim = c(1, 10), ylim = c(0, 1), 
       main = "Distribution of detection delay for RSPRT", ylab = "P(reject null | null is false)", xlab = "detection lag")

delay_wt_low <- test_sprt_low %>% ungroup() %>%
  select(time, R0_true, simulation, decision_wt) %>%
  group_by(simulation) %>%
  mutate(detect_time = detect_index(decision_wt, is_h1),
         detect_time = ifelse(detect_time > 0, detect_time, 10^6),
...
```{r eval = FALSE}
delay_regular <- test_sprt %>% ungroup() %>%
  select(time, R0_true, simulation, decision) %>%
  group_by(simulation) %>%
  mutate(detect_time = detect_index(decision, is_h1),
         detect_time = ifelse(detect_time > 0, detect_time, 10^6),
         true_time = detect_index(R0_true, is_thresh_R)) %>%
  summarize(delay = unique(detect_time - true_time)) 

delay_regular %>% pull(delay) %>% ecdf() %>% 
  plot(xlim = c(1, 10), ylim = c(0, 1), 
       main = "Distribution of detection delay for SPRT", ylab = "proportion of simulations", xlab = "detection lag")
```

The following plots are included for comparison to previous results - most of the important information can be understood from the detection delay distribtuion plots. But, these plots do show a major different between RSPRT and WSPRT: upon $H_1$
 beocming true, most RSPRT decision sequences have an instance of uncertainty whereas the WSPRT decision sequences (falsly) accept $H_0$ for another timestep (or two) which reflects the difference between how past evidence influences current decisions for each test.
```{r comparison_line_plots, out.width="50%", fig.show="hold", warning=FALSE}
# test_sprt %>%
#   ggplot() +
#   geom_line(aes(x = time, y = infected, group = simulation)) +
#   geom_point(aes(x = time, y = infected, group = simulation, col = decision)) +
#   geom_vline(aes(xintercept = which(r_linear == 0.7)), linetype = "dashed", size = 1.5) +
#   geom_vline(aes(xintercept = which(r_linear == 1)), size = 1.5) +
#   scale_y_log10() +
#   viridis::scale_color_viridis(discrete = TRUE) +
...
test_sprt %>%
  ggplot() +
  geom_line(aes(x = time, y = infected, group = simulation), alpha = 0.5) +
  geom_jitter(aes(x = time, y = infected, group = simulation, col = decision_reset), alpha = 0.5) +
  geom_vline(aes(xintercept = which(r_linear == 0.7)), linetype = "dashed", size = 1.5) +
  geom_vline(aes(xintercept = which(r_linear == 1)), size = 1.5) +
  scale_y_log10() +
  viridis::scale_color_viridis(discrete = TRUE) +
  labs(title = "RSPRT",#title = "Outbreak sizes and decisions",
       y = expression(New~infections~(x[t])),
       x = "Outbreak generation (t)",
       col = "Decision") + 
  my_theme()
...
test_sprt %>%
  ggplot() +
  geom_line(aes(x = time, y = infected, group = simulation), alpha = 0.5) +
  geom_jitter(aes(x = time, y = infected, group = simulation, col = decision_wt), alpha = 0.5) +
  geom_vline(aes(xintercept = which(r_linear == 0.7)), linetype = "dashed", size = 1.5) +
  geom_vline(aes(xintercept = which(r_linear == 1)), size = 1.5) +
  scale_y_log10() +
  viridis::scale_color_viridis(discrete = TRUE) +
  labs(title = "WSPRT",#title = "Outbreak sizes and decisions",
       y = expression(New~infections~(x[t])),
       x = "Outbreak generation (t)",
       col = "Decision") + 
  my_theme()
...
```{r comparison_bar_plots,out.width="50%", fig.show="hold", warning=FALSE}
# test_sprt %>%
#   group_by(time) %>%
#   count(decision) %>%
#   complete(nesting(decision = c("?", "H0", "H1")), fill = list(n = 0)) %>%
#   ggplot() +
#   geom_bar(aes(x = time, y = n, fill = decision), stat = "identity", position = "fill", width = 1) +
#   geom_vline(aes(xintercept = which(r_linear == 0.7)), linetype = "dashed", size = 1.5) +
#   geom_vline(aes(xintercept = which(r_linear == 1)), size = 1.5)  +
#   labs(title = "d",#title = "Conclusions of of reset SPRT over time",
#        #subtitle = "H0 is true from t = 1:10; H1 is true from t =11:20",
...
test_sprt %>%
  ...
  labs(title = "RSPRT",#title = "Conclusions of of reset SPRT over time",
       #subtitle = "H0 is true from t = 1:10; H1 is true from t =11:20",
       y = "Proportion of simulations",
       x = "Outbreak generation (t)",
       fill = "Decision") +
...
test_sprt %>%
  group_by(time) %>%
  count(decision_wt) %>%
  complete(nesting(decision_wt = c("?", "H0", "H1")), fill = list(n = 0)) %>%
  ggplot() +
  geom_bar(aes(x = time, y = n, fill = decision_wt), stat = "identity", position = "fill", width = 1) +
  geom_vline(aes(xintercept = which(r_linear == 0.7)), linetype = "dashed", size = 1.5) +
  geom_vline(aes(xintercept = which(r_linear == 1)), size = 1.5)  +
  viridis::scale_fill_viridis(discrete = TRUE) +
  labs(title = "WSPRT",#title = "Conclusions of of reset SPRT over time",
       #subtitle = "H0 is true from t = 1:10; H1 is true from t =11:20",
       y = "Proportion of simulations",
       x = "Outbreak generation (t)",
       fill = "Decision") +
...
* Define set of performance metrics
    + Calculate distributions of lag times for rejecting $H_0$
    + what else? ROC?
    + performance plots could include:
        + drift in $R_t$
        + decisions for each simulated trajectory
        + proportion of simluations where tests correctly rejected null during simulation length
        + distribution of decision times (could be problematic to calculate if RSPRT doesn't always reject in finite time)
...
# see methods section of Merl et al. (2009) https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0005807
...
out <- matrix(NA, nrow = length(time), ncol = 4)
out[,1] <- time
out[1,2] <- s0
out[1,3] <- i0
out[1, 4] <- 0 # no recovered


i <- 1
while(i < tail(time,1) & out[i,3] > 0) {
  # calculate probability of a susceptible (resp. infected) becomeing infected (resp. recovering)
  prob_inf <- (1 - (k / (k + beta * out[i,3])) ^ k)
  prob_rec <- (1 - exp(-gamma))
  # draw number of infected and recovered
  infected <- rbinom(1, size = out[i,2], prob = prob_inf )
  recovered <- rbinom(1, size = out[i,3], prob = prob_rec)
  # update data
  out[i + 1, 2] <- out[i, 2] - infected
  out[i + 1, 3] <- out[i, 3] + infected - recovered
  out[i + 1, 4] <- out[i, 4] + recovered
...
out <- 
  out %>%  pivot_longer(cols = 2:4,
                      names_to = "class",
                      values_to = "number")

ggplot(out, aes(x = time, y = number, col = class)) +
  geom_line()

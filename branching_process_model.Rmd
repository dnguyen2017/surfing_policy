...
set.seed(123)
testdf <- episim(t = t, i_0 = i_0, r0 = r0, disp = disp, cont = cont, nsim = 100)
...
# Linearly decreasing $R_t$

How do the sequential tests perform when $R_t$ decreases linearly and we want to test the hypothesis that $H_0: \theta_t > \theta_0 \text{ against } H_1: \theta_t \leq \theta_0$ where the threshold $0 < \theta_0 = 1 - c(t) < 1$. That is, when is transmission low enough that repoening will not result in $R > 1$.
...
set.seed(123)
df_lin <- episim(t = t, i_0 = 10^2, r0 = r_linear, disp = disp, cont = cont, nsim = 100)
```

```{r plot_lin_rt, warning = FALSE}
df_lin %>%
  ggplot() +
  geom_line(aes(x = time, y = infected, group = simulation)) +
  geom_vline(aes(xintercept = which(r_linear == 0.7)), col = "red", linetype = "dashed") +
  geom_vline(aes(xintercept = which(r_linear == 1))) +
  scale_y_log10()
```

```{r calc_sprt_lineardecrease}
lin_sprt <- calc_sr(df_lin, accept_reg = acceptance, reject_reg = rejection)
...
lin_sprt <- calc_reset_sr(lin_sprt)
...
lin_sprt %>%
  ...
  ggplot() +
  geom_bar(aes(x = time, y = n, fill = decision_reset), stat = "identity", position = "fill") +
  geom_vline(aes(xintercept = which(r_linear == 0.7)), col = "red", linetype = "dashed", size = 1.5) +
  geom_vline(aes(xintercept = which(r_linear == 1)), size = 1.5) 
```

**Figure 7: Performance of reset SPRT when** $\boldsymbol{R_t}$ **decreases linearly**

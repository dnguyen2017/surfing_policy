...
X_{t+1} & \sim \Sigma^{Z_t}_{i=1}X_i \\
            & \sim NB(\Sigma^{X_t}_{i=1} R(t), \Sigma^{X_t}_{i=1} k) \text{     (I think this is correct. still need to prove)}
\end{align*}
...
```{r plot_example_branching_proc, out.width = "50%", fig.show = 'hold'}
# Plot simulations
...
ggplot(df, aes(x = day, y = n, col = inf_day)) +
  geom_point() + viridis::scale_color_viridis() +
  labs(title = "Simulated cases per day",
